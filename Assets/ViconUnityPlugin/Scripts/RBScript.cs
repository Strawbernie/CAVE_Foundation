...
namespace UnityVicon
{
    public class RBScript : MonoBehaviour
    {
        ...
        public Quaternion trackerRotationOffset;
        ...
        void Update()
        {
            ...
            if (ORot.Result == Result.Success && OTran.Result == Result.Success && !OTran.Occluded)
            {
                ...
                Root.localRotation = new Quaternion((float)ORot.Rotation[1], -(float)ORot.Rotation[3], -(float)ORot.Rotation[0], -(float)ORot.Rotation[2]);
                //Root.localPosition = new Vector3(-(float)OTran.Translation[1] * 0.001f, (float)OTran.Translation[2] * 0.001f, (float)OTran.Translation[0] * 0.001f) + trackerPositionOffset;
                Root.localPosition = new Vector3(-(float)OTran.Translation[0] * 0.001f, (float)OTran.Translation[2] * 0.001f, -(float)OTran.Translation[1] * 0.001f) + trackerPositionOffset;
